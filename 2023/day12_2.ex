defmodule Main do
   def main(path) do
      ...
      symbol_rows = input |> Enum.map(fn x -> hd(String.split(x, " ")) |> String.graphemes end) |> Enum.map(fn x -> x++["?"]++x++["?"]++x++["?"]++x++["?"]++x end)
      number_rows = input |> Enum.map(fn x -> hd(tl(String.split(x, " "))) |> String.split(",") |> Enum.map(&String.to_integer/1) end) |> Enum.map(fn x -> x++x++x++x++x end)
