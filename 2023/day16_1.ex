defmodule Main do
    ...
    |> Enum.reduce([%{}, 0], fn x, [map, index] ->
      [
        Map.put_new(
          map,
          index,
          Enum.reduce(String.graphemes(x), [%{}, 0], fn z, [map2, index2] ->
            [Map.put_new(map2, index2, z), index2 + 1]
          end)
          |> hd()
        ),
        index + 1
  ...
  def follow_beam(map, visited, direction, {x, y}) do
    case Main.check_index(map, {x, y}) do
      false ->
        visited

      true ->
        case Enum.member?(Main.get_item(visited, {x, y}), direction) do
          true ->
            visited
            ...
            case Main.get_item(map, {x, y}) do
              "." ->
                Main.follow_beam(
                  ...
                  new_visited,
                  direction,
                  Main.update_coords({x, y}, direction)
                )
  ...
  def add_item(map, {x, y}, new_direction) do
    Map.update(map, y, %{x => new_direction}, fn inner_map ->
      Map.update(inner_map, x, new_direction, fn old_directions ->
        old_directions ++ new_direction
      end)
    end)
  end

  def update_coords({x, y}, direction) do
    case direction do
      "r" -> {x + 1, y}
      "l" -> {x - 1, y}
      "d" -> {x, y + 1}
